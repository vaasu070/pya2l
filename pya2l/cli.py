...
def main():
    ...
    json.add_argument('input_file', nargs=1, help='full path to input file')
    json.add_argument('-o', nargs=1, help='full path to output file')
    ...
    if args.sub_command == JSON_CMD:
        with open(args.o[0] if args.o is not None else args.input_file[0] + '.json', 'wb') as fp:
            dump(p.ast.json, fp, indent=4, sort_keys=True, ensure_ascii=False)
